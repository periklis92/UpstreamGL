...
template <class EventType> using EventDelegate = Delegate<bool(const EventType *)>;

template <class EventType> class EventDispatcher {
...
public:
  ...
  void Subscribe(delegate_type callback) { m_Delegates.push_back(callback); }
  ...
  void Publish(const EventType &e) { Publish(&e); }

private:
  std::vector<delegate_type> m_Delegates;
};
} // namespace upgl