...
glm::mat4 AnimationChannel::InterpolatePosition(float time) const {
  if (m_PositionKeys.size() == 1)
    return glm::translate(glm::mat4(1.f), m_PositionKeys[0].Position);
  if (m_PositionKeys.size() > 1) {
    int32_t index0 = GetPosIndex(time);
    int32_t index1 = index0 + 1;

    float t = GetFactor(m_PositionKeys[index0].TimeStamp, m_PositionKeys[index1].TimeStamp, time);

    glm::vec3 finalPos =
        glm::mix(m_PositionKeys[index0].Position, m_PositionKeys[index1].Position, t);
...
glm::mat4 AnimationChannel::InterpolateRotation(float time) const {
  if (m_RotationKeys.size() == 1)
    return glm::mat4_cast(glm::normalize(m_RotationKeys[0].Rotation));
  if (m_PositionKeys.size() > 1) {
    int32_t index0 = GetRotIndex(time);
    int32_t index1 = index0 + 1;

    float t = GetFactor(m_RotationKeys[index0].TimeStamp, m_RotationKeys[index1].TimeStamp, time);

    glm::quat finalRot =
        glm::slerp(m_RotationKeys[index0].Rotation, m_RotationKeys[index1].Rotation, t);
...
glm::mat4 AnimationChannel::InterpolateScale(float time) const {
  if (m_ScaleKeys.size() == 1)
    return glm::scale(glm::mat4(1.f), m_ScaleKeys[0].Scale);
  if (m_ScaleKeys.size() > 1) {
    int32_t index0 = GetScaleIndex(time);
    int32_t index1 = index0 + 1;

    float t = GetFactor(m_ScaleKeys[index0].TimeStamp, m_ScaleKeys[index1].TimeStamp, time);

    glm::vec3 finalScale = glm::mix(m_ScaleKeys[index0].Scale, m_ScaleKeys[index1].Scale, t);
