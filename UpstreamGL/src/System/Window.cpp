...
namespace upgl {
Window::Window(const char *title, WindowSettings settings) : m_GLFWHandle(nullptr) {
  ...
  m_GLFWHandle =
      glfwCreateWindow(settings.Resolution.Width, settings.Resolution.Height, title, NULL, NULL);

  UPGL_ASSERT(m_GLFWHandle);
...
bool Window::ShouldClose() { return glfwWindowShouldClose(m_GLFWHandle); }

void Window::MakeCurrent() { glfwMakeContextCurrent(m_GLFWHandle); }

void Window::SetFullscreen(bool fullscreen) {
  if (!fullscreen) {
    glfwSetWindowMonitor(m_GLFWHandle, NULL, 32, 32, m_Settings.Resolution.Width - 32,
                         m_Settings.Resolution.Height - 32, m_Settings.Resolution.RefreshRate);
...
void Window::SetResolution(const WindowResolution &resolution) {
  if (!m_Settings.Fullscreen) {
    ...
    glfwSetWindowMonitor(m_GLFWHandle, NULL, x, y, resolution.Width, resolution.Height,
    ...
    glfwSetWindowMonitor(m_GLFWHandle, monitor, 0, 0, resolution.Width, resolution.Height,
                         resolution.RefreshRate);
  }
...
void Window::__KeyCallback(GLFWwindow *glwin, int key, int, int action, int) {
  ...
  UPGL_ASSERT(window);
  InputManager::GetInstance()->OnKeyboardKey().Publish(
      InputKeyboardEvent{(KeyboardKey)key, action == GLFW_PRESS ? true : false});
}
...
void Window::__MouseButtonCallback(GLFWwindow *glwin, int button, int action, int) {
  Window *window = static_cast<Window *>(glfwGetWindowUserPointer(glwin));
  UPGL_ASSERT(window);
  InputManager::GetInstance()->OnMouseButton().Publish(
      InputMouseButtonEvent{(MouseButton)button, action == GLFW_PRESS ? true : false});
}

void Window::__MousePosCallback(GLFWwindow *glwin, double x, double y) {
  Window *window = static_cast<Window *>(glfwGetWindowUserPointer(glwin));
  UPGL_ASSERT(window);
  InputManager::GetInstance()->OnMouseMove().Publish(
      InputMouseMoveEvent{(float)x - window->m_LastMouseX, (float)y - window->m_LastMouseY});
